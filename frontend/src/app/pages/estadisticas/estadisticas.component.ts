...
import {
  ...
  LinearScale,
  Point,
} from "chart.js"; // Importa la escala lineal de Chart.js
...
export class EstadisticasComponent implements OnInit {
  ...
  selectedValue: string = "dia"; // Inicializar con un valor predeterminado para el mat-select
  ...
  ngOnInit(): void {
    this.estudianteService.obtenerTotalEstudiantes().subscribe(
      (total: number) => {
        // Especificar el tipo de 'total' como number
  ...
  cambiarRangoFechas(rango: string): void {
    ...
    switch (rango) {
      case "dia":
        this.startDate = new Date(
          hoy.getFullYear(),hoy.getMonth(),hoy.getDate(),0,0,0); // Medianoche
        this.endDate = new Date(
          hoy.getFullYear(),hoy.getMonth(),hoy.getDate(),23,59,59); // Final del día
        diasEnPeriodo = 1;
        break;
      case "semana":
        this.startDate = new Date(
          hoy.getFullYear(),hoy.getMonth(),hoy.getDate() - hoy.getDay());
        this.endDate = new Date(
          hoy.getFullYear(),hoy.getMonth(),hoy.getDate() + (6 - hoy.getDay()));
        diasEnPeriodo = 7;
        break;
      case "mes":
  ...
  inicializarPastel(asistencias: any[]): void {
    ...
    const asistenciasPorCategoria = asistencias.reduce(
    ...
    const estudiantesAsistidosEnCategoria =
      asistenciasPorCategoria[this.selectedCategory] || 0;
    const estudiantesSinAsistenciaEnCategoria =
      this.totalEstudiantesPeriodo - estudiantesAsistidosEnCategoria;

    this.data = [
      estudiantesAsistidosEnCategoria,
      estudiantesSinAsistenciaEnCategoria,
    ];
  ...
  inicializarBarras(asistencias: any[]): void {
    const asistenciasPorCursoYAlmuerzo = asistencias.reduce(
      (acumulador, asistencia) => {
        const curso = asistencia.estudiante.curso;
        const almuerzoNombre =
          asistencia.almuerzo.nombre.charAt(0).toUpperCase() +
          asistencia.almuerzo.nombre.slice(1).toLowerCase(); // Asegura consistencia en nombres
        if (!acumulador[curso]) {
          acumulador[curso] = {
            Almuerzo: 0,
            Desayuno: 0,
    ...
    const dataBarrasFormatted: {
      [curso: string]: { [almuerzo: string]: number };
    } = {};
  ...
  inicilizarLinea(asistencias: any[], startDate: Date, endDate: Date): void {
    const recuentoAsistencias: {
      [fecha: string]: { [tipoComida: string]: number };
    } = {};
    ...
    asistencias.forEach((asistencia) => {
      ...
      const tipoComida =
        asistencia.almuerzo.nombre.charAt(0).toUpperCase() +
        asistencia.almuerzo.nombre.slice(1).toLowerCase();

      if (!recuentoAsistencias[fecha]) {
        recuentoAsistencias[fecha] = {
          Desayuno: 0,
          Refrigerio: 0,
          Almuerzo: 0,
        };
  ...
  obtenerAsistencias(): void {
    ...
    if (this.startDate && this.endDate) {
      ...
      console.log("Fechas:", this.endDate + "//"+ this.startDate); // Verifica los datos aquí

      diasEnPeriodo = Math.round((Math.abs(this.endDate.getTime() - this.startDate.getTime()))/ (1000 * 3600 * 24));

      console.log("DiasEnPeriodo:", diasEnPeriodo); // Verifica los datos aquí
  ...
  actualizarCategoriasDisponibles(asistencias: any[]): void {
    ...
    const categoriasUnicas = Array.from(
      new Set(asistencias.map((asistencia) => asistencia.almuerzo.nombre))
    );
